...
def build_patch(session_dict, fq=250, n_win=2):
    sym_sz = ['bckg', 'fnsz', 'gnsz', 'spsz', 'cpsz', 'absz', 'tnsz', 'tcsz', 'mysz']
    win_len = fq * n_win
    seg, lbl = [], []
    for edf_file, lbl_file in session_dict.values():
        ...
        for key in ann_dict.keys():
            ...
            if key not in ['FP1-F7']: continue #specified channel
            ...
            if st_i !=-1 and ed_i !=-1:
                ...
                for cls, st, ed in ann_dict[key]:
                    ...
                    if sym_sz.index(cls) == 0 and lbl.count(0)/(len(lbl)+1)>0.50: #keep non-sz balance
                        continue 
