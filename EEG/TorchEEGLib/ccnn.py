...
#https://github.com/torcheeg/torcheeg/blob/main/torcheeg/models/cnn/ccnn.py
...
if __name__ == "__main__":
    x = torch.rand(10, 18, 256).to("cuda:5")
    x = x.reshape(x.size(0), x.size(1), 16, 16)
    model = CCNN(in_channels=18, grid_size = (16, 16), num_classes=2).to("cuda:5")
    out = model(x)
    print(out.shape)