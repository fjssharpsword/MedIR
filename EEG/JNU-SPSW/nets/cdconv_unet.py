...
class ADConv_1D(nn.Module): #average difference convolution
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 padding=1, dilation=1, groups=1, bias=False, theta=0.7):

        super(ADConv_1D, self).__init__() 
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, groups=groups, bias=bias)
        self.avgpool = nn.AvgPool1d(kernel_size, stride=1, padding=1)
        self.theta = theta
...
class conv_block(nn.Module):
    def __init__(self, in_c, out_c):
        super().__init__()
        self.conv1 = CDConv_1D(in_c, out_c, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm1d(out_c)
        self.conv2 = CDConv_1D(out_c, out_c, kernel_size=3, padding=1)
...
if __name__ == "__main__":
    ...
    inputs = torch.randn((10, 1, 250)).to(device)
