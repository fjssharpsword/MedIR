...
#https://github.com/microsoft/FocalNet/blob/main/segmentation/mmseg/models/backbones/focalnet.py
if __name__ == "__main__":
    device = torch.device("cuda:4" if torch.cuda.is_available() else "cpu")
    #inputs = torch.randn((8, 3, 224, 224)).to(device)
    #model = FocalNet().to(device)
    inputs = torch.randn((8, 224, 224, 3)).to(device)
    model = FocalModulation(dim=3).to(device)
    y = model(inputs)
    print(y.shape)