...
class EEGDGCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        '''
        x = x.permute(0, 2, 1) #graph on chunk data
...
if __name__ == "__main__":
    ...
    x = torch.rand(10, 1, 250).cuda()
    #model = EEGDGCNN(in_channels = 512, num_electrodes = 18, num_classes=2).cuda() #graph on electrodes
    model = EEGDGCNN(in_channels = 1, num_electrodes = 250, num_classes=1).cuda() #graph on chunk data
    out = model(x)
    print(out.shape)