...
'''
   同一样本组下的AUC配对比较
   z = (A1-A2)/sqrt(se^2(A1)+se^2(A2)-2Cov(A1,A2))
   大样本下，z服从正太分布
'''
...
if __name__ == '__main__':
    csv_path1 = "/data/Metrics/temp/nlst_once_cnn.csv"
    csv_path2 = "/data/Metrics/temp/nlst_once_radiomics.csv"
    #gt, pred
    df1 = pd.read_csv(csv_path1)
    y_true = df1['label'].values 
    y_pred_1 = df1['pred'].values 

    df2 = pd.read_csv(csv_path2)
    y_pred_2 = df2['pred'].values 
    #pvalue
    pvalue = delong_roc_test(y_true,y_pred_1,y_pred_2)
    aucs1, ci1 = delong_roc_ci(y_true, y_pred_1, alpha=0.95)
    aucs2, ci2 = delong_roc_ci(y_true, y_pred_2, alpha=0.95)

    print('95% AUC CI of AUC1:', aucs1, ci1)
    print('95% AUC CI of AUC2:', aucs2, ci2)
